...
for ride in rides:
    ...
    for i in range(F):
        ...
        if nexttime <= ride.e:
            if idx < 0: # for c
                idx = i
            elif dist(ride.a, ride.b, taxis[idx].x, taxis[idx].y) > dist(ride.a, ride.b, taxis[i].x, taxis[i].y):
                idx = i 
            """
            elif taxis[idx].t < taxis[i].t: #1
                idx = i
            """
            """
            elif nexttime <= ride.s: #3
                ...
                idx_nexttime = taxis[idx].t + dist(ride.a, ride.b, taxis[idx].x, taxis[idx].y)
                if idx_nexttime > ride.s and taxis[idx].t < taxis[i].t:
                    idx = i
            """
            """
            elif taxis[idx].t > taxis[i].t: #2
                idx = i
            """
            
    if idx == -1:
        #print("failed to ride" + str(ride.idx))
        continue
    
    ride.ok = True
    taxis[idx].Rs.append( ride.idx )
    nexttime = taxis[idx].t + dist(ride.a, ride.b, taxis[idx].x, taxis[idx].y)
    if nexttime>=ride.s:
